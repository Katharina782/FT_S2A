...
## For training Basenji2', the buggy architecture

# activation before the residual connection
...
for epoch in range(options.num_epochs):
    ...
    for i, batch in enumerate(zip(dataloader["train_human"], dataloader["train_mouse"])):
        for h, head in enumerate(["human", "mouse"]):
            ...
            # save loss
            if head == "human":
                train_loss_epoch_human.append(loss.item())
            else:
                train_loss_epoch_mouse.append(loss.item())
            optimizer.step()
    ...
    with torch.no_grad():
        for i, batch in enumerate(zip(dataloader["valid_human"], dataloader["valid_mouse"])):
            for h, head in enumerate(["human", "mouse"]):
                ...
                loss = poisson_loss(out, target)
                if head == "human":
                    test_loss_epoch_human.append(loss.item())
                else:
                    test_loss_epoch_mouse.append(loss.item())
