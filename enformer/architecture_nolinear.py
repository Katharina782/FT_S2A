...
def poisson_loss_bp(pred, target):
...
class TargetLengthCrop(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("computing Target Length Crop")
        if target_len == -1:
            return x
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = logits.softmax(dim = -1)
        # this is the attention that one can plot in the end
...
class Enformer(PreTrainedModel):
    # this defines all the parameters as set in the config file
    ...
    def __init__(self, config):
        ...
        self.stem = nn.Sequential(
            nn.Conv1d(in_channels=4, out_channels=half_dim, kernel_size=15, padding = 7),
    ...
    def forward(
        ...
        # for key, value in heads dictionary compute function of value
        ...
        # if we provide targets and head directly to the forward pass
        # we can actually return the loss from the forward pass!
        if exists(target):
            assert exists(head), 'head must be passed in if one were to calculate loss directly with targets'
            
            # this print statement was used to check the multi-gpu training
            #print(x.shape, target.shape)
            if return_corr_coef: 
                return pearson_corr_coef(out, target)
            
            if return_loss_per_bp:
                return poisson_loss_bp(out, target)
